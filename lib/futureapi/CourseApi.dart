...
Future<List<Course>> getAllCourses() async {
  Uri url = Uri.parse('http://192.168.1.5/api/webCourse.php?status=data');
...
Future<List<Course>> getCourseByID(int courseID) async {
  Uri url = Uri.parse('http://192.168.1.5/api/webCourse.php?status=one&id=$courseID');
  final response = await http.get(url);
  if (response.statusCode == 200) {
    List<Course> courses = [];
...
Future<List<Course>> getCourseView() async {
  final response = await http.get(Uri.parse(
      'http://192.168.1.5/api/walid/courseapi.php?operation=mostview'));
  if (response.statusCode == 200) {
    List<Course> courses = [];
    dynamic jsonResponse = json.decode(response.body);
    if (jsonResponse is List) {
...
Future<List<Course>> getCourseNew() async {
  Uri url = Uri.parse('http://192.168.1.5/api/walid/courseapi.php?operation=latest');
  final response = await http.get(url);
  if (response.statusCode == 200) {
    List<Course> courses = [];
    dynamic jsonResponse = json.decode(response.body);
...
Future<List<Course>> getRandomCourse() async {
  final Uri url = Uri.parse('http://192.168.1.5/api/walid/courseapi.php?operation=random');
  final response = await http.get(url);
  if (response.statusCode == 200) {
