...
Future<List<Trainer>> fetchTrainers(int userId) async {
    final response = await http.get(Uri.parse(
        'http://192.168.1.12/api/walid/userapi.php?status=SelectOne&UserID=$userId'));
...
Future<List<Trainer>> fetchAllTrainers() async {
  final response = await http.get(Uri.parse('http://192.168.1.12/api/walid/userapi.php?status=UserData'));
